...
# ==========================================================
# 1. ログインガード（ここが最優先：チャット画面を物理的に隠す）
# ==========================================================
# ページ設定はログイン画面の見た目にも影響するため、一番上で定義します
st.set_page_config(page_title="M25", page_icon="💬", layout="wide")

if "password_correct" not in st.session_state:
    st.session_state["password_correct"] = False

# ログインしていない場合は、チャット画面の全要素をブラウザ側で「真っ白」に塗りつぶす
if not st.session_state["password_correct"]:
    # 💡 魔法の一手：チャットのデザインが漏れ出さないよう、画面全体を「白」で上書き固定
    st.markdown("""
        <style>
            /* チャット画面の背景色や要素をすべて強制的に隠す */
            .stApp { background-color: white !important; color: black !important; }
            [data-testid="stVerticalBlock"] { background-color: white !important; }
            header, footer { visibility: hidden !important; }
            /* パスワード入力欄の文字を黒で見やすくする */
            div[data-testid="stMarkdownContainer"] p { color: black !important; font-size: 1.2rem; }
            input { color: black !important; border: 1px solid #ccc !important; }
        </style>
    ...
    # 💡 重要：ここで完全に止める。これ以降のコード（演出CSS含む）は1行も読ませない。
    st.stop()

# ==========================================================
# 2. 認証成功後のみ読み込まれるメインコード（安全地帯）
# ==========================================================

# 演出に必要なライブラリをここでインポート
